...
std::vector<Coord> Character::calculateAttackRange(const int gridWidth, const int gridHeight) {
    ...
    // Up
    if (pos.y > 0) {
        attackRange.push_back({pos.x, pos.y - 1});
    }
    ...
    // Left
    if (pos.x > 0) {
        attackRange.push_back({pos.x - 1, pos.y});
    }
